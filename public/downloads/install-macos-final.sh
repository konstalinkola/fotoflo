...
echo "🚀 Installing Fotoflo Desktop Sync for macOS (Final Working Version)..."
echo ""
...
upload_file() {
    ...
    # Upload using curl with proper file handling
    RESPONSE=$(curl -s -w "\n%{http_code}" -X POST \
        -F "file=@\"$file\"" \
...
echo "🔍 Scanning for images to upload..."
cd "$FOLDER"
...
# Monitor for new files using a simple approach
while true; do
    cd "$FOLDER"
    ...
    for file in *.jpg *.jpeg *.png *.webp *.gif; do
        if [ -f "$file" ] 2>/dev/null; then
            ...
            if [ ! -f "$PROCESSED_FILES" ] || ! grep -q "^$full_path$" "$PROCESSED_FILES" 2>/dev/null; then
                # Check if file is new (modified in last 10 seconds)
                if [ $(find "$file" -mtime -10s 2>/dev/null | wc -l) -gt 0 ]; then
                    upload_file "$full_path"
                fi
            fi
        fi
...
echo ""
echo "💡 This version:"
echo "   • Simple file detection"
echo "   • Handles spaces in filenames"
echo "   • Uploads existing images first"
echo "   • Monitors for new images"
