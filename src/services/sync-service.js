...
class FotofloSyncService {
  ...
  async detectServerUrl() {
    // Priority order for server URL detection - prioritize production domain
    const candidates = [
    ...
    // Fallback - try production domain first, then localhost for development
    const fallback = 'https://fotoflo.vercel.app';
